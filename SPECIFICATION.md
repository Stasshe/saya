...
リポジトリ現在空(README.mdのみ)。Rust単一バイナリで新規実装。

### 確定設計判断(ユーザー対話で確定)

- **sudo方針**: 当初案「`saya apply`一般ユーザー実行、内部で個別sudo呼ぶ」から変更。`sudo saya apply`含め全サブコマンドsudo実行前提。理由: 「sudo apt installが多く、OSレベル操作はsudoが普通」。設定ファイルは`$HOME/.config/saya/packages.toml`(rootの`$HOME`=`/root`でなく元ユーザーhome)に保存→`SUDO_UID`から元ユーザー解決し所有権戻す処理は残る。
...
└── src/
    ...
    ├── privilege.rs       # SUDO_UID解決・home取得・require_root・chown
    ├── backend/
...
- `Manifest::save(path)` — atomic write(`.tmp`書き込み→`fs::rename`)。chownはmanifest.rsの責務外、呼び出し側(`commands/`)が`privilege::resolve_original_user()`の結果で行う。
- `Manifest::find_logical_name_by_real(real_name, kind)` — shim記録時、既存論理名の再利用判定に使う。

### privilege.rs
```rust
pub struct OriginalUser { pub uid: u32, pub gid: u32, pub home: PathBuf }
pub fn require_root() -> Result<()>;
pub fn resolve_original_user() -> Result<OriginalUser>; // SUDO_UID優先、無ければSUDO_USER
pub fn chown_to_user(path: &Path, user: &OriginalUser) -> Result<()>;
```
`SUDO_UID`優先理由: 数値なので名前変更・特殊文字の曖昧さ無い。`getpwuid_r`/`getpwnam_r`はunsafeブロック1箇所に閉じ込める。`main.rs`のディスパッチ層で`require_root()`を一度だけ呼ぶ。
...
4. 既存マニフェストに論理名あれば再利用、無ければ実パッケージ名そのまま新規論理名として追加、atomic write+chown
5. 元の終了コードで`process::exit`
...
- `add.rs` / `forget`: manifestへのキー追加・削除、save。対称操作なので1ファイルにまとめる。
- `import.rs`: `list_manually_installed`から未収録分を一覧。`--edit`ならtmpfileに書き出し`$EDITOR`(無ければ`vi`)起動、編集後マージ。`--edit`無しは候補を標準出力するだけで保存しない(誤操作防止の2段階確認)。

### cli.rs / main.rs
clap derive。`main.rs`で`argv[0]`のbasenameを見て`shim::run`か`run_saya_cli`に振り分け。shim経路は`Cli::parse()`を通さない(aptの引数をclapに渡さない)。`run_saya_cli`内で`privilege::require_root()`を呼んでから各サブコマンドへdispatch。

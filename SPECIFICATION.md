...
- **マニフェスト書き込み**: `0644`で保存する。保存内容が既存ファイルと同一なら内容を書き換えず、権限のみ補正する。差分があればatomic write(tmpファイル→rename)する。file lockingは初期版では作らない。
...
`saya install`(パッケージ引数なし)はmanifestの各エントリについて `is_installed` を確認し、未インストールの実パッケージ名だけをまとめて `install()` に渡す。`saya install <name...>`は検出中backendで全指定パッケージを一度にインストールし、コマンド全体の成功後に未記録名を配列へ追記する。全て記録済みならインストールだけ行いmanifestは保存しない。`--`以降の引数は両方の形式でbackendのinstall引数へ順序と値を変えずに渡し、manifestには記録しない。
...
- install command(複数指定/manifest一括)の成功時記録・未導入パッケージ抽出・backend引数境界
